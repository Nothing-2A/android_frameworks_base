...
public class UdfpsController implements DozeReceiver, Dumpable {
    ...
    private boolean mUseMtkGhbmDimming = mContext.getResources().getBoolean(
    ...
    private void onFingerDown(
        ...
        if (mUseMtkGhbmDimming && mOverlay != null) {
            ...
            if (hbmView != null && hbmParams != null) {
                int brightness = MtkUdfpsScrimController.getInstance().getSystemBrightness(mContext);
                float alpha = MtkUdfpsScrimController.calculateAlpha(brightness);
